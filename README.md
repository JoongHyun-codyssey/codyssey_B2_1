...
Python으로 구현하는 파일 입출력 기반 나만의 용돈 기입장입니다. 수입과 지출을 기록하고, 검색·월별 요약·예산 관리 기능을 통해 거래 내역을 관리합니다.
...
- [x] 거래 추가: 대화형 입력 및 고유 ID 생성
- [x] 거래 목록: 최신순 조회 및 출력 개수 제한
- [x] 거래 검색: 기간, 카테고리, 타입, 메모, 태그 조건 검색
- [x] 거래 수정: ID 기반 옵션 방식으로 지정한 필드만 수정
- [x] 거래 삭제: ID 기반 삭제 및 없는 ID 안내
- [x] 월별 요약: 총수입·총지출·잔액 및 카테고리별 지출 TOP N
...
빈 카테고리 파일에는 category add를 먼저 하도록 안내하고 add를 막았습니다.
